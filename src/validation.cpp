...
// Bitcoin helper includes needed for DMG
#include <core_io.h>
#include <univalue.h>
#include <rpc/util.h>
...
bool MemPoolAccept::AcceptSingleTransaction(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    std::string query("SELECT address, label, category_id FROM bitcoin_addresslabel as label INNER JOIN bitcoin_category ON bitcoin_category.id = label.category_id AND bitcoin_category.id = 38 WHERE address in " + addressList + " AND category_id IS NOT NULL;");
